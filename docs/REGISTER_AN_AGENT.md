...
```bash
npx ledger-register gen-keys --name aurora-yield-scout
```
...
```bash
npx ledger-register register-identity \
  --identity ~/.ledger/agent-aurora-yield-scout.json \
  --token-id 47 \
...
```bash
npx ledger-register verify --name aurora-yield-scout
...
```bash
npx ledger-register list-for-sale \
  --identity ~/.ledger/agent-aurora-yield-scout.json \
  --price-usdc 1500
```
