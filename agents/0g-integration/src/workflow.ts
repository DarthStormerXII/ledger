...
import { runReasoning, type LedgerModel, type ReasoningResult } from "../../0g-compute/src/index.js";
...
export async function assertMemoryRoundTrip(input: {
  ...
  const storage = input.storage ?? new MemoryStorageAdapter();
  const { cid } = await uploadAgentMemory(input.agentId, input.plaintext, storage);
  const output = await downloadAgentMemory(cid, storage);
  if (!output.equals(input.plaintext)) throw new Error("0G memory roundtrip failed byte equality");
  return { cid, byteEqual: true };
}

export async function runWorkerReasoning(prompt: string, model: LedgerModel = "0g-qwen3.6-plus"): Promise<ReasoningResult> {
  return runReasoning(prompt, model);
}

export async function readDemoWorker(config: ZeroGIntegrationConfig = loadZeroGConfig()): Promise<WorkerProfile> {
