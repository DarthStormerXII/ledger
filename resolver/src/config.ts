...
export function loadConfig(
  ...
  const ogChainId = Number.parseInt(
    env.OG_CHAIN_ID ?? env.GALILEO_CHAIN_ID ?? "16602",
    10,
  );
  const workerInftAddressValue =
    env.WORKER_INFT_ADDRESS ?? env.WORKER_INFT_CONTRACT;
  ...
  return {
    ...
    ogRpcUrl:
      env.OG_RPC_URL ?? env.GALILEO_RPC ?? "https://evmrpc-testnet.0g.ai",
    ...
    mockOwnerAddress,
    txReceipts: parseJsonMap<TxReceiptRecord>(
      env.TX_RECEIPTS_JSON,
      "TX_RECEIPTS_JSON",
    ),
    repSummaries: parseJsonMap<ReputationSummary>(
      env.REP_SUMMARIES_JSON,
      "REP_SUMMARIES_JSON",
    ),
    memoryPointers: parseJsonMap<string>(
      env.MEMORY_POINTERS_JSON,
      "MEMORY_POINTERS_JSON",
    ),
    ...
    ttlSeconds:
      Number.isFinite(ttlSeconds) && ttlSeconds >= 0 ? ttlSeconds : 30,
  };
}
