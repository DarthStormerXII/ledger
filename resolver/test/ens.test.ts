...
const config: ResolverConfig = {
  ...
  privateKey:
    "0x59c6995e998f97a5a0044966f094538d4f78304485ea2b58e07890d97097369b",
  txReceipts: {
    "task-001": {
      cid: "0g://receipt-cid",
...
describe("ENS gateway", () => {
  it("encodes who.* addr responses", async () => {
    ...
    const data = resolverInterface.encodeFunctionData("addr(bytes32)", [
      namehash(name),
    ]);
    const result = await query(dnsEncode(name), data, config);

    expect(
      resolverInterface.decodeFunctionResult("addr(bytes32)", result)[0],
    ).toBe(config.mockOwnerAddress);
  });
  ...
  it("encodes who.* coinType 60 addr responses", async () => {
    ...
    const result = await query(dnsEncode(name), data, config);
    const encodedAddress = resolverInterface.decodeFunctionResult(
  ...
  it("signs IResolverService responses for the OffchainResolver callback", async () => {
    const name = "who.worker-001.ledger-demo.eth";
    const innerData = resolverInterface.encodeFunctionData("addr(bytes32)", [
      namehash(name),
    ]);
    ...
    const [result, expires, sig] = serviceInterface.decodeFunctionResult(
      "resolve",
      response.data,
    );
  ...
  it("returns tx, rep, and mem text records", async () => {
    await expectText(
      "tx.task-001.worker-001.ledger-demo.eth",
      "ai.tx.intent",
      "Base Yield Scout",
    );
    await expectText("rep.worker-001.ledger-demo.eth", "ai.rep.count", "47");
    await expectText(
      "mem.worker-001.ledger-demo.eth",
      "ai.mem.cid",
      "0g://memory-cid",
    );
  });
  ...
  it("returns the parent ENSIP-25 agent-registration record", async () => {
    expect(await text("ledger-demo.eth", "agent-registration")).toContain(
      "ENSIP-25",
    );
  });

  it("verifies HD derivation chains from an account xpub", async () => {
    const master = await text(
      "pay.worker-099.ledger-demo.eth",
      "ai.pay.master",
    );
    const child1 = await text(
      "pay.worker-099.ledger-demo.eth",
      "ai.pay.address",
    );
    const nonce1 = BigInt(
      await text("pay.worker-099.ledger-demo.eth", "ai.pay.nonce"),
    );
    const child2 = await text(
      "pay.worker-099.ledger-demo.eth",
      "ai.pay.address",
    );
    const nonce2 = BigInt(
    ...
    // worker-099 → tokenId 99 (resolver derives at path `<tokenId>/<nonce>`)
    expect(verifyDerivation(master, child1, child2, nonce1, nonce2, 99n)).toBe(
      true,
    );
  });
});
