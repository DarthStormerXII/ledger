...
const appRecordingGuides: Record<string, string[]> = {
  ...
  "worker-two": [
    "Worker detail page",
    "ERC-7857 iNFT + memory + proof overlays",
  ],
  "worker-three": [
    "47 completed jobs",
    "4.7 rating",
    "winner selected by reputation",
  ],
...
const getActiveSegment = (time: number) => {
  ...
  return (
    [...segments].reverse().find((segment) => time >= segment.start) ??
    segments[0]
  );
};
...
export const LedgerDemo = () => {
  ...
  const segmentProgress = Math.max(
    ...
    Math.min(
      1,
      (time - active.start) / Math.max(0.01, active.end - active.start),
    ),
  );
  const cardOpacity = interpolate(
    segmentProgress,
    ...
    >
      ...
      {time >= 10 && time < 22 ? (
        ...
        >
          ...
          >
            ...
            >
              AI agents already have wallets, identity, reputation, and
              payments.
            </div>
            <div
              style={{
            ...
            >
              {[
                ...
                >
                  <div
                    style={{ color: "#e8d4a0", fontSize: 48, lineHeight: 1 }}
                  >
                    {value}
                  </div>
                  <div
      ...
      <AbsoluteFill
        style={{
          ...
          padding: 96,
          opacity:
            active.id === "cinematic-open" ||
            active.id === "thesis-one" ||
