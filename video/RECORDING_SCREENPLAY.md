# Ledger Demo Video Recording Screenplay

Current cut: approximately 3:32 total.

This document covers the parts that still need real app/demo footage. The opening, thesis background, Ledger title card, sponsor proof section, and outro are already handled in Remotion/Higgsfield.
...
Audio: "AI agents now have wallets. They have identity. They have reputation. But they still do not have a marketplace."

Status: Done.

### 0:09-0:22 — Thesis Market Gap

Visual: `thesis-background.mp4`, with Remotion data panel showing:

- 21,000+ agents
- 100M+ agent payments
- Zero places where agents can really hire each other

Status: Done.

### 0:22-0:28 — Ledger Thesis Card

Visual: Remotion Ledger product-unveil card.

Audio: "Ledger is the hiring hall for AI agents, and the workers themselves are the assets."
...
Audio: "This is Ledger. A market where AI workers can be hired, verified, transferred, and owned. Live on testnet today."

Status: Done.

## Required App Recordings

### 0:29-0:41 — Worker Part 1: Buyer Posts Work

Goal: Show Ledger as a labor market where agents hire agents.
...
Goal: Prove the worker is not just an app profile.
...
This section should communicate: this worker has state, memory, proofs, and ownership.
...
Goal: Show ERC-8004 reputation being used to choose the winning worker.

Record:

Stay on the selected worker, or open a reputation tab. Show completed jobs, average rating, feedback records, and signed/verifiable reputation entries if available. Then return to the bid list and show this worker being selected because it is qualified.
...
Hold the reputation stats long enough to read. Then show market selection. The important idea is that reputation is not decoration; it affects who wins work.

### 1:26-1:45 — ENS Capability Identity Part 1

Goal: Show ENS as the stable name and capability interface.
...
- Right side: resolved capability records.
...
Goal: Show the name staying stable while state changes.
...
Goal: Create a beat before the market mechanics.
...
Goal: Show the full labor-market loop.
...
Goal: Introduce ownership transfer.
...
Goal: Prove transfer does not reset the worker.
...
Goal: Show ownership changes through ENS resolution.

Record:

Open ENS resolution panel for `who.worker-001.ledger.eth`. Show before owner wallet, then after transfer show new owner wallet. The narration says no ENS transaction, so if the UI can show "resolved dynamically" or "CCIP-read/live resolver," include that.
...
Goal: Deliver the product punchline before sponsor proof.
